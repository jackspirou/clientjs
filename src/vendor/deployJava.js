    ...
    getJREs: function() {
      ...
      if (this.debug) {
        for (var j = 0; j < t.length; ++j) {
          g("[getJREs()] We claim to have detected Java SE " + t[j]);
        }
      }
      return t;
    },
    ...
    versionCheck: function(p) {
      ...
      if (y !== null) {
        ...
        for (var j = 0; j < w.length; ++j) {
          if (this.compareVersionToPattern(w[j], q, r, u)) {
            return true;
          }
        }
        return false;
      ...
      if (navigator.mimeTypes !== null) {
        ...
        if (!o) {
          for (var w = 0; w < navigator.mimeTypes.length; w++) {
            if (navigator.mimeTypes[w].type == this.oldMimeType) {
              if (navigator.mimeTypes[w].enabledPlugin) {
