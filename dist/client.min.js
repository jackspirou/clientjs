...
0;h=(16777619*h^a[l])>>>0;g.update(a[l]);if(l===e-1||g.sum()%f===f-1)c[0]+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(d&63),d=671226215;if(l===e-1||g.sum()%(2*f)===2*f-1)c[1]+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(h&63),h=671226215}return c}function q(a,b){return 100*(1-e(a,b)/Math.max(a.length,b.length))}var c={},d=!1;"undefined"!==typeof module&&module.exports?exports=module.exports=c:(this.ctph=c,d=!0);b.prototype.update=function(a){this.y-=
this.x;this.y+=7*a;this.x+=a;this.x-=this.window[this.c%7]||0;this.window[this.c%7]=a;this.c++;this.z=this.z<<5>>>0;this.z=(this.z^a)>>>0};b.prototype.sum=function(){return this.x+this.y+this.z>>>0};c.digest=function(k){if("string"===typeof k)if(d){k=unescape(encodeURIComponent(k));for(var b=[],c=0;c<k.length;c++)b.push(k.charCodeAt(c));k=b}else k=(new Buffer(k)).toJSON();b=Math.ceil(Math.log(k.length/192)/Math.log(2));b=Math.max(3,b);for(c=a(k,3<<b);0<b&&32>c[0].length;)c=a(k,3<<--b);return"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(b)+
